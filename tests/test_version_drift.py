...
Single source of truth for DocHarvest version: ``11.0.8``.
...
- ``docs/lib/version.ts`` and ``src/gitbook_downloader/__init__.py`` are the
  canonical sources and MUST equal ``11.0.8`` (we assert equality, not just
  presence).
- ``frontend/index.html`` <title> must read ``DocHarvest v11.0.8``.

Drift signals (these MUST all read ``11.0.8`` after Phase 1 step 1):
...
# Files where the file MUST contain the literal ``11.0.8`` somewhere.
...
def test_cli_version_fallback_uses_canonical_value() -> None:
    """The direct-script fallback in cli.py MUST equal 11.0.8 (not 9.0.0b1)."""
    ...
    assert "__version__ = \"11.0.8\"" in cli_text, (
        "cli.py direct-script fallback should be 11.0.8, not 9.0.0b1"
    )
...
def test_python_version_importable() -> None:
    """Smoke: the Python package exposes __version__ == 11.0.8."""
...
# hand on the 11.0.8 bump: five `11.0.6` literals hardcoded in frontend
