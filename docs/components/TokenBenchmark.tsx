  ...
  return (
    <div className="mt-12 border-t border-rule-strong pt-10">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
        <div>
          <p className="sheet-label">TOKEN ECONOMY &amp; BENCHMARK</p>
          <h3 className="sheet-head mt-2 text-ink">
            ~{STATS.reductionPct}% Token Reduction. Zero HTML/JSX Soup.
          </h3>
        </div>
        <div className="flex items-center gap-2">
          <span className="sheet-num text-[12px] text-match font-semibold">
      ...
      <div className="mt-8 border border-rule-strong bg-bond-2 p-6">
          ...
          <div>
            <div className="flex items-baseline justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="sheet-num text-[11px] text-ink-3">({rawScraper.totalBytes})</span>
              </div>
              <span className="sheet-num text-[13px] text-ink-2 font-medium">
          ...
          <div>
            <div className="flex items-baseline justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="sheet-num text-[11px] text-match/80">({docHarvest.totalBytes})</span>
              </div>
              <span className="sheet-num text-[13px] text-match font-semibold">
            ...
            <div className="mt-2.5 flex h-4 w-full border border-rule overflow-hidden bg-bond">
              ...
              >
                <span className="sheet-label text-[9px] text-match truncate px-1">
                  83% NOISE ELIMINATED
                </span>
              </div>
            </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 border-t border-rule pt-6 sm:grid-cols-4">
          <div>
            <span className="sheet-label text-[10px]">Token Savings</span>
            <p className="sheet-num mt-1 text-[20px] font-bold text-ink">
              ~{STATS.reductionPct}%
            </p>
            <p className="sheet-num text-[11px] text-ink-3">82.8% measured</p>
          </div>
          <div>
            <span className="sheet-label text-[10px]">Tokens Stripped</span>
            <p className="sheet-num mt-1 text-[20px] font-bold text-match">
              {docHarvest.tokensSaved.toLocaleString()}
            </p>
            <p className="sheet-num text-[11px] text-ink-3">per doc snapshot</p>
          </div>
          <div>
            <span className="sheet-label text-[10px]">Byte Reduction</span>
            <p className="sheet-num mt-1 text-[20px] font-bold text-ink">
              312 KB → 34 KB
            </p>
            <p className="sheet-num text-[11px] text-ink-3">89.1% smaller</p>
          </div>
          <div>
            <span className="sheet-label text-[10px]">Context Window Cost</span>
            <p className="sheet-num mt-1 text-[20px] font-bold text-match">
              $0.00
            </p>
        ...
        >
          <button
            ...
            className={`sheet-label cursor-pointer border-r border-rule px-4 py-2 text-[11px] transition-colors focus-visible:outline-2 focus-visible:outline-match ${
              activeView === 'comparison' ? 'bg-bond text-match border-t-2 border-t-match' : 'text-ink-3 hover:text-ink'
            }`}
          >
          ...
          <button
            ...
            className={`sheet-label cursor-pointer border-r border-rule px-4 py-2 text-[11px] transition-colors focus-visible:outline-2 focus-visible:outline-match ${
              activeView === 'noiseBreakdown' ? 'bg-bond text-match border-t-2 border-t-match' : 'text-ink-3 hover:text-ink'
            }`}
          >
          ...
          <button
            ...
            className={`sheet-label cursor-pointer border-r border-rule px-4 py-2 text-[11px] transition-colors focus-visible:outline-2 focus-visible:outline-match ${
              activeView === 'preservedContent' ? 'bg-bond text-match border-t-2 border-t-match' : 'text-ink-3 hover:text-ink'
            }`}
          >
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="border border-rule bg-bond p-4">
                ...
                <ul className="mt-3 space-y-2 sheet-body text-[12px] text-ink-2">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-alert font-mono">✗</span>
                    <span>40KB+ of client hydration bundles (<code className="sheet-num text-ink">__NEXT_DATA__</code>) filling context limits.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-alert font-mono">✗</span>
              ...
              <div className="border border-rule bg-bond p-4">
                ...
                <ul className="mt-3 space-y-2 sheet-body text-[12px] text-ink-2">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-match font-mono">✓</span>
                    <span>Structure-aware <code className="sheet-num text-ink">read_doc</code> section bounding that never breaks mid-block.</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <table className="w-full border-collapse text-left">
              ...
              <thead>
                <tr className="border-b border-rule-strong">
                  <th scope="col" className="sheet-label py-2 pr-4 font-medium">Noise Category</th>
                  <th scope="col" className="sheet-label py-2 pr-4 font-medium">Share</th>
                  <th scope="col" className="sheet-label py-2 pr-4 font-medium">Tokens Wasted</th>
                  <th scope="col" className="sheet-label py-2 font-medium">Impact Description</th>
                </tr>
              </thead>
              <tbody>
                {rawScraper.noiseBreakdown.map((item) => (
                  <tr key={item.category} className="border-b border-rule">
                    <th scope="row" className="sheet-term py-2.5 pr-4 text-[13px] font-normal text-ink">
                      {item.category}
                    </th>
                    <td className="sheet-num py-2.5 pr-4 text-[13px] text-ink-2">
                      {item.percentage}%
                    </td>
                    <td className="sheet-num py-2.5 pr-4 text-[13px] text-alert font-medium">
        ...
        {activeView === 'preservedContent' && (
          ...
          >
            <table className="w-full border-collapse text-left">
              ...
              <thead>
                <tr className="border-b border-rule-strong">
                  <th scope="col" className="sheet-label py-2 pr-4 font-medium">Semantic Structure</th>
                  <th scope="col" className="sheet-label py-2 pr-4 font-medium">Token Allocation</th>
                  <th scope="col" className="sheet-label py-2 font-medium">Compiler Guarantee</th>
                </tr>
              </thead>
              <tbody>
                {docHarvest.preservedContent.map((item) => (
                  <tr key={item.category} className="border-b border-rule">
                    ...
                    <td className="sheet-num py-2.5 pr-4 text-[13px] text-match font-medium">
                      {item.tokens.toLocaleString()} tokens
                    </td>
                    <td className="sheet-body py-2.5 text-[12px] text-ink-2">
                      {item.description}
