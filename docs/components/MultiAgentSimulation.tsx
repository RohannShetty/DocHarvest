  ...
  return (
    <div className="mt-12 border-t border-rule-strong pt-10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between">
        <div>
          <p className="sheet-label text-match">AUTONOMOUS MULTI-AGENT SIMULATION</p>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="sheet-num inline-flex cursor-pointer items-center gap-1.5 border border-rule bg-bond-2 px-3 py-1.5 text-[11px] text-ink transition-all hover:border-match active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-match"
          >
            {isPlaying ? (
              <>
      ...
      <p className="sheet-body mt-3 max-w-[68ch] text-ink-2">
        ...
        autonomous coding agents dispatch <code className="sheet-num text-ink font-mono text-xs bg-bond px-1 py-0.5 border border-rule">download_docs</code>,{' '}
        <code className="sheet-num text-ink font-mono text-xs bg-bond px-1 py-0.5 border border-rule">search_docs</code>,{' '}
        <code className="sheet-num text-ink font-mono text-xs bg-bond px-1 py-0.5 border border-rule">query_doc_graph</code>, and{' '}
        <code className="sheet-num text-ink font-mono text-xs bg-bond px-1 py-0.5 border border-rule">read_doc</code> with zero context overflow.
      </p>
        ...
        {MULTI_AGENT_SIMULATION_STEPS.map((s, index) => {
          ...
          return (
            <button
              ...
              className={`flex cursor-pointer flex-col gap-1 border-b border-r border-rule p-4 text-left transition-all focus-visible:outline-2 focus-visible:outline-match ${
                isActive ? 'bg-bond-2 border-t-2 border-t-match' : 'bg-bond hover:bg-bond-2/50'
              }`}
            >
              <div className="flex items-center justify-between">
                ...
                <span className="sheet-label text-[10px] text-ink-3">
              ...
              <p className="sheet-num text-[11px] text-ink-3 truncate font-mono">
      ...
      <div
        ...
        tabIndex={0}
        className="border border-t-0 border-rule-strong bg-bond-2 p-6 shadow-sm"
      >
        {/* Step details header */}
        <div className="grid gap-6 lg:grid-cols-12">
          <div className="lg:col-span-7">
            ...
            <p className="sheet-body mt-2 text-[13px] text-ink-2 leading-relaxed">{step.purpose}</p>

            {/* Tool call declaration */}
            <div className="mt-4 border border-rule bg-bond p-3.5">
              <div className="flex items-center justify-between">
                <span className="sheet-label flex items-center gap-1.5 text-ink">
                  <Terminal aria-hidden="true" className="h-3.5 w-3.5 text-match" />
                  Tool Invocation
                </span>
                <span className="sheet-num text-[11px] text-match font-mono font-medium">{step.toolCall.toolName}</span>
              </div>
              <pre className="mt-2 text-[12px] text-ink overflow-x-auto p-2.5 bg-bond-2 border border-rule font-mono leading-relaxed">
          ...
          <div className="flex flex-col justify-between border-t border-rule pt-4 lg:col-span-5 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
            <div>
              <span className="sheet-label flex items-center gap-1.5 text-match">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-rule pt-3">
                <div>
                  <dt className="sheet-label text-[10px] text-ink-3">Latency</dt>
                  <dd className="sheet-num text-[14px] font-semibold text-ink font-mono">{step.telemetry.latency}</dd>
                </div>
                <div>
                  <dt className="sheet-label text-[10px] text-ink-3">Status</dt>
                  <dd className="sheet-term text-[12px] text-match font-medium">{step.telemetry.status}</dd>
                </div>
                <div>
                  <dt className="sheet-label text-[10px] text-ink-3">Prompt Tokens</dt>
                  <dd className="sheet-num text-[13px] text-ink-2 font-mono">{step.telemetry.tokensIn} tokens</dd>
                </div>
                <div>
                  <dt className="sheet-label text-[10px] text-ink-3">Emitted Tokens</dt>
                  <dd className="sheet-num text-[13px] text-ink font-mono font-medium">{step.telemetry.tokensOut} tokens</dd>
                </div>
                <div className="col-span-2">
                  <dt className="sheet-label text-[10px] text-ink-3">AST Precision Bound</dt>
                  <dd className="sheet-num text-[12px] text-ink-2 font-mono">{step.telemetry.astBound}</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="mt-4 border-t border-rule pt-3">
              <span className="sheet-label flex items-center gap-1.5 text-[10px] text-ink-3">
                <Layers aria-hidden="true" className="h-3 w-3 text-ink-3" />
                Verified Outputs
              </span>
              <ul className="mt-2 space-y-1">
                {step.artifactsProduced.map((art) => (
                  <li key={art} className="sheet-num text-[11px] text-ink-3 flex items-center gap-1.5 truncate font-mono">
        ...
        <div className="mt-6 border-t border-rule pt-4">
          <div className="flex items-baseline justify-between mb-2">
            <span className="sheet-label text-ink-2">stdio verification transcript</span>
