...
describe('version constants', () => {
  it('should export VERSION as 11.1.0', () => {
    expect(VERSION).toBe('11.1.0');
  });

  it('should export DOWNLOAD_URLS with correct Windows URL', () => {
    expect(DOWNLOAD_URLS.windows).toBe(
      `https://github.com/RohannShetty/DocHarvest/releases/download/v${VERSION}/docharvest-windows-latest.exe`
    );
  });

  it('should export DOWNLOAD_URLS with correct Linux URL', () => {
    expect(DOWNLOAD_URLS.linux).toBe(
      `https://github.com/RohannShetty/DocHarvest/releases/download/v${VERSION}/docharvest-ubuntu-latest`
    );
  });

  it('should export DOWNLOAD_URLS with correct macOS URL', () => {
    expect(DOWNLOAD_URLS.macos).toBe(
      `https://github.com/RohannShetty/DocHarvest/releases/download/v${VERSION}/docharvest-macos-latest`
    );
  });
