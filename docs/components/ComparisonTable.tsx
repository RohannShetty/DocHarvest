...
function Cell({ value, highlight }: { value: boolean | string; highlight: boolean }) {
  if (value === true) {
    return <span className={highlight ? 'text-match font-bold text-base' : 'text-ink font-semibold'}>✓</span>;
  }
  if (value === false) {
    return <span className="text-ink-3 text-base">✗</span>;
  }
  return <span className="sheet-label text-ink-2 font-medium">{value}</span>;
}
    ...
    >
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
          <div>
            <p className="sheet-label text-match">05 — THE COMPARISON</p>
          ...
          <table className="w-full border-collapse text-left">
            ...
            <thead>
              <tr className="border-b border-rule-strong bg-bond">
                <th scope="col" className="sheet-label py-4 px-4 font-medium text-ink">
                ...
                {COLUMNS.map((column) => (
                  <th
                    scope="col"
                    key={column.key}
                    className={`sheet-label py-4 px-4 text-center font-medium ${
                      column.highlight ? 'text-match font-semibold bg-match/10 border-x border-match/20' : 'text-ink-2'
                    }`}
                  >
                    {column.label}
                  </th>
            ...
            <tbody>
              {MATRIX_ROWS.map((row) => (
                <tr key={row.feature} className="border-b border-rule align-top hover:bg-bond/30 transition-colors">
                  <th scope="row" className="py-4 px-4 text-left font-normal">
                    <span className="sheet-term text-[14px] text-ink font-medium">{row.feature}</span>
                    <span className="sheet-label mt-1 block text-ink-3">{row.detail}</span>
                  </th>
                  {COLUMNS.map((column) => (
                    <td
                      key={column.key}
                      className={`py-4 px-4 text-center text-[14px] ${
                        column.highlight ? 'bg-match/5 border-x border-match/20' : ''
                      }`}
                    >
